...
if (-not (Get-Command "gh" -ErrorAction SilentlyContinue)) {
  Write-Error "GitHub CLI (gh) not found in PATH."
  exit 1
}
...
try {
  ...
  if (-not $repo) {
    throw "Could not retrieve repository information."
  }
  Write-Host "Target Repo: $repo"
}
catch {
  Write-Error "Failed to get repository info. Ensure you are authenticated with 'gh auth login' and in a git repository. Error: $_"
  exit 1
}
...
foreach ($labelName in $labels.Keys) {
  Write-Host "Checking Label: $labelName"
  # Try to create label, ignore if it exists (gh label create fails if exists)
  gh label create "$labelName" --color "$($labels[$labelName])" --description "Priority: $labelName" 2>&1 | Out-Null
}
...
foreach ($milestone in $ghParams.Keys) {
  ...
  # We remove Out-Null to see errors if any
  gh api "repos/$repo/milestones" -f title="$milestone" 2>&1
  if ($LASTEXITCODE -ne 0) {
    Write-Warning "Failed to create milestone '$milestone'. It might already exist."
  }

  foreach ($issue in $ghParams[$milestone]) {
    Write-Host "  Creating Issue: $($issue.Title)"
    # We remove Out-Null to see errors if any
    gh issue create --title "$($issue.Title)" --body "$($issue.Body)" --label "$($issue.Label)" --milestone "$milestone" 2>&1
    if ($LASTEXITCODE -ne 0) {
